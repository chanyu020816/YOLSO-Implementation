...
class YOLSODataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        target = torch.zeros(
            (self.output_grid_size, self.output_grid_size, self.num_classes + 4) # number of classes + bg class + x, y, s
        )
        target[..., self.num_classes] = 1 # initialize all target as background class
        for box in boxes:
            ...
            # if no object already found in cell i, j, set to 0
            if target[i, j, self.num_classes] == 1:
                target[i, j, self.num_classes] = 0 # bg class set to 0, indicate this grid has symbol
            ...
            else: # cell i, j already has an object -> grid size is too large
                # warnings.warn(f"Grid cell ({i}, {j}) already has an object. The grid size may be set too large.")
                pass
        return torch.tensor(np.array(image)).permute(0, 1, 2), target.permute(2, 0, 1)
